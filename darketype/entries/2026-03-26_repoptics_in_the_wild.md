...
- **port.io write-back:** i was introduced to internal developer portals (idps), specifically port.io, and it opened up a whole new layer. repoptics now has a context-aware floating widget that allows users to push their live scan results directly back to their port.io entity. it turns the tool from a read-only mirror into a bidirectional control surface for organizational health.
